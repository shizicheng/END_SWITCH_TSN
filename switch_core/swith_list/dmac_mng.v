module dmac_mng #(
        ...
        input               wire        [PORT_NUM-1:0]                  i_rx_port                               , // 接收端口bitmap,每个bit代表??个端??
        
        /*----------------------------- 查表输出接口 ------------------------------*/                           
        output              wire                                        o_dmac_lookup_vld                       , // DMAC查表结果有效
        output              wire        [PORT_NUM-1:0]                  o_dmac_tx_port                          , // DMAC查表结果：转发端?? 
        output              wire                                        o_dmac_lookup_hit                       , // DMAC查表命中标志
        output              wire                                        o_lookup_clash                          , // 查表冲突标志：哈希地??有表项但MAC/VLAN不匹??
        ...
        input              wire         [31:0]                          i_age_scan_interval                     , // 老化扫描间隔配置寄存器（秒）
...
/*---------------------------------------- 寄存器地??定义 -------------------------------------------*/
//localparam  REG_AGE_TIME_THRESHOLD      = 8'h00                             ; // 老化时间阈???配置寄存器
//localparam  REG_TABLE_CLEAR             = 8'h01                             ; // MAC表清空寄存器
//localparam  REG_TABLE_FULL_THRESHOLD    = 8'h02                             ; // MAC表满阈???配置寄存器
//localparam  REG_AGE_SCAN_INTERVAL       = 8'h03                             ; // 老化扫描间隔配置寄存??
...
/*---------------------------------------- 内部信号定义 -------------------------------------------*/
// 输入数据FIFO相关参数和信??
localparam  INPUT_FIFO_DEPTH            = 8                                 ; // 输入FIFO深度，支??8个报文缓??
...
wire                                    w_fifo1_wr_en                       ; // FIFO1写使??
wire        [FIFO1_WIDTH-1:0]           w_fifo1_din                         ; // FIFO1输入数据
wire                                    w_fifo1_full                        ; // FIFO1满标??
wire                                    w_fifo1_rd_en                       ; // FIFO1读使??
wire        [FIFO1_WIDTH-1:0]           w_fifo1_dout                        ; // FIFO1输出数据
wire                                    w_fifo1_empty                       ; // FIFO1空标??

// FIFO2: DMAC
wire                                    w_fifo2_wr_en                       ; // FIFO2写使??
wire        [FIFO2_WIDTH-1:0]           w_fifo2_din                         ; // FIFO2输入数据
wire                                    w_fifo2_full                        ; // FIFO2满标??
wire                                    w_fifo2_rd_en                       ; // FIFO2读使??
wire        [FIFO2_WIDTH-1:0]           w_fifo2_dout                        ; // FIFO2输出数据
wire                                    w_fifo2_empty                       ; // FIFO2空标??

// FIFO3: SMAC  
wire                                    w_fifo3_wr_en                       ; // FIFO3写使??
wire        [FIFO3_WIDTH-1:0]           w_fifo3_din                         ; // FIFO3输入数据
wire                                    w_fifo3_full                        ; // FIFO3满标??
wire                                    w_fifo3_rd_en                       ; // FIFO3读使??
wire        [FIFO3_WIDTH-1:0]           w_fifo3_dout                        ; // FIFO3输出数据
wire                                    w_fifo3_empty                       ; // FIFO3空标??

// FIFO4: DMAC_HASH_ADDR + DMAC_HASH_VLD
wire                                    w_fifo4_wr_en                       ; // FIFO4写使??
wire        [FIFO4_WIDTH-1:0]           w_fifo4_din                         ; // FIFO4输入数据
wire                                    w_fifo4_full                        ; // FIFO4满标??
wire                                    w_fifo4_rd_en                       ; // FIFO4读使??
wire        [FIFO4_WIDTH-1:0]           w_fifo4_dout                        ; // FIFO4输出数据
wire                                    w_fifo4_empty                       ; // FIFO4空标??

// FIFO5: SMAC_HASH_ADDR + SMAC_HASH_VLD
wire                                    w_fifo5_wr_en                       ; // FIFO5写使??
wire        [FIFO5_WIDTH-1:0]           w_fifo5_din                         ; // FIFO5输入数据
wire                                    w_fifo5_full                        ; // FIFO5满标??
wire                                    w_fifo5_rd_en                       ; // FIFO5读使??
wire        [FIFO5_WIDTH-1:0]           w_fifo5_dout                        ; // FIFO5输出数据
wire                                    w_fifo5_empty                       ; // FIFO5空标??

// 综合FIFO状???信??
wire                                    w_all_fifo_full                     ; // ??有FIFO满标??
wire                                    w_all_fifo_empty                    ; // ??有FIFO空标??
...
wire        [PORT_NUM-1:0]              w_fifo_rx_port                      ; // 接收端口bitmap
...
reg                                     r_lookup_clash                      ; // 查表冲突标志寄存??

// 状???机相关
reg         [3:0]                       r_fsm_cur_state                     ;
reg         [3:0]                       r_fsm_nxt_state                     ;
reg         [15:0]                      r_state_cnt                         ; // 状???计数器，计数每个状态保持时??
...
// 寄存器读取MAC表的独立通道
reg         [HASH_DATA_WIDTH-1:0]       r_reg_table_raddr                   ; // 寄存器读表地??
reg                                     r_reg_table_rd                      ; // 寄存器读表使??
...
reg                                       r_dmac_list_full_er_stat            ; // DMAC列表满过标志
reg           [15:0]                      r_dmac_list_full_er_cnt             ; // DMAC列表满过计数??

// 统计寄存??
reg         [31:0]                      r_learn_success_cnt               ; // 学习成功计数??
reg         [31:0]                      r_learn_fail_cnt                  ; // 学习失败计数??
reg         [31:0]                      r_collision_cnt                   ; // 哈希冲突计数??
reg         [31:0]                      r_port_move_cnt                   ; // 端口移动计数??

// 表项计数相关
reg         [14:0]                      r_table_entry_cnt                   ; // MAC表有效表项计数器（最??32768??
reg                                     r_entry_add                         ; // 表项添加标志
reg                                     r_entry_del                         ; // 表项删除标志
wire                                    w_table_full                        ; // 表满状???信??
wire                                    w_age_scan_trigger                  ; // 老化扫描触发信号
reg                                     r_table_full                        ; // 表满状???信??
// 老化相关信号
reg                                     r_age_scan_en                       ; // 老化扫描使能
reg                                     r_age_timer_pulse                   ; // 老化定时器脉冲（1秒脉冲）
reg         [AGE_TIME_WIDTH-1:0]        r_global_timestamp                  ; // 全局时间戳计数器（秒??
reg         [7:0]                       r_clear_burst_cnt                   ; // 清表突发计数器，限制连续清表操作??
reg                                     r_agescan_cnt                       ; // 老化地址保持计数??
reg         [HASH_DATA_WIDTH-1:0]       r_age_scan_breakpoint               ; // 老化扫描断点地址

// 分级时间计数器相关信??
reg         [15:0]                      r_us_cnt                            ; // 微秒计数器（1-65535us，支持不同时钟频率）
reg         [9:0]                       r_ms_cnt                            ; // 毫秒计数??
reg         [31:0]                      r_s_cnt                             ; // 毫秒总计数器（用于内??1秒脉冲生成）
...
localparam  CLEAR_BURST_LIMIT           = 8'd16                             ; // 清表突发限制：连续清??16个地??后让位给数据包处??  
...
/*---------------------------------------- 输入数据FIFO管理 -------------------------------------------*/
// 上升沿检测寄存器
reg                                     r_dmac_hash_vld_d1                  ; // DMAC哈希有效信号延迟????
reg                                     r_smac_hash_vld_d1                  ; // SMAC哈希有效信号延迟????
reg                                     r_entry_expired_flag                ;
// 上升沿检测???辑
wire                                    w_dmac_hash_vld_posedge             ; // DMAC哈希有效上升??
wire                                    w_smac_hash_vld_posedge             ; // SMAC哈希有效上升??
wire                                    w_both_hash_valid                   ; // 两个hash都有??
wire                                    w_new_packet_arrival                ; // 新报文到达信??
...
// 5个FIFO的读使能：同时读取，只有当所有FIFO都不空时才读??
...
reg         [PORT_NUM-1:0]              r_cur_rx_port                       ; // 接收端口bitmap

// STD模式FIFO读使能延迟寄存器
reg                                     r_fifo_rd_en_d1                     ; // 读使能延迟一??
...
// STD模式：在rd_en的下??个周期数据才有效，所以使用延迟的rd_en信号 
...
/*---------------------------------------- bitmap有效性检?? ----------------------------------------------*/
// ??查r_cur_rx_port是否为有效的one-hot编码(只有??个bit??1)或全0
wire                                    w_rx_port_valid                     ; // 接收端口bitmap有效标志
...
// 老化??测???辑??  当前时间戳减去表项时间戳的差值大于等于???化阈???时认为过期
...
always @(*) begin
    ...
    case (r_fsm_cur_state)
        IDLE: 
            r_fsm_nxt_state = (i_table_clear_req == 1'd1) ? AGE_SCAN :       // 清表优先级最??
                              // ??查FIFO是否有数据可处理或有直连数据可用
                              (r_can_use_direct_path == 1'd1) ? DMAC_LOOKUP :  // 直连路径跳过等待状???
                              (w_all_fifo_empty == 1'd0) ? FIFO_READ_WAIT :    // STD模式??要等待数据准??
        ...
        AGE_SCAN: 
            r_fsm_nxt_state = (r_mac_table_addr == {HASH_DATA_WIDTH{1'b1}}) ? IDLE :    // 扫描完成（包括清表完成）
                              ((w_new_packet_arrival == 1'd1) && ((i_table_clear_req == 1'd0) || (r_clear_burst_cnt >= CLEAR_BURST_LIMIT))) ? IDLE : // 有报文等待且(非清表模?? ?? 已达到突发限??)时才优先处理报文
                              ((w_entry_valid == 1'd1) && (w_entry_expired == 1'd1) && (i_table_clear_req == 1'd0) && (r_state_cnt >= 16'd1)) ? AGE_UPDATE :    // ??要???化更新（非清表模式），确保数据稳定
                              AGE_SCAN;                                              // 继续扫描下一个地??        
        AGE_UPDATE: 
            r_fsm_nxt_state = (r_mac_table_addr == {HASH_DATA_WIDTH{1'b1}}) ? IDLE :  // 扫描完成
                              ((w_all_fifo_empty == 1'd0) && ((i_table_clear_req == 1'd0) || (r_clear_burst_cnt >= CLEAR_BURST_LIMIT))) ? IDLE : // 有报文等待且(非清表模?? ?? 已达到突发限??)时才优先处理报文
                              AGE_SCAN;                                                // 老化更新完成，继续扫??        
        default: 
...
always @(posedge i_clk or posedge i_rst) begin
    if (i_rst) begin
        ...
        r_age_scan_breakpoint <= {HASH_DATA_WIDTH{1'b0}};  // 扫描完成时重置断??
    end
end
        ...
        case (r_fsm_cur_state)
            IDLE: begin
                if (r_can_use_direct_path == 1'd1||w_can_use_direct_path == 1'd1) begin
                    ...
                    // STD模式：不能立即使用FIFO输出，需要等读取后再在FIFO_READ_WAIT状???设??
                    r_mac_table_addr <= {HASH_DATA_WIDTH{1'b0}};  // 先清??
                end else begin
                    r_mac_table_addr <= {HASH_DATA_WIDTH{1'b0}};
                end
            end
            FIFO_READ_WAIT: begin  // 在此状???设置从锁存数据中获取的地址
                r_mac_table_addr <= r_cur_dmac_hash_addr;  // 使用已锁存的数据
            end
            DMAC_LOOKUP: begin 
                r_mac_table_addr <= r_state_cnt == 16'd1 &&  w_dmac_match == 1'd0 ? r_cur_smac_hash_addr : r_cur_dmac_hash_addr;  // 使用已锁存的数据
            end
            DMAC_REFRESH: begin
            ...
            AGE_SCAN, AGE_UPDATE: begin
                if (r_fsm_cur_state == IDLE && (r_fsm_nxt_state == AGE_SCAN)) begin
                    r_mac_table_addr <= r_age_scan_breakpoint;  // 从断点恢复或??0????
...
ram_simple2port #(
    ...
    .clka           ( i_clk                                 ), 
    .clkb           ( i_clk                                 ), 
    .wea            ( r_mac_table_we                        ), // 学习
    .enb            ( r_mac_table_re                        ), // 查表
    .rstb           ( i_rst                                 ), 
    .regceb         ( 1'b0                                  ),
